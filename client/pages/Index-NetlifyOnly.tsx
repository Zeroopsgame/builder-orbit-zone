...
export default function Index() {
  // Initialize Netlify Blobs storage - ONLY storage method
  const getCrewStore = () => {
    try {
  ...
  // Helper function to save crew data - ONLY Netlify Blobs
  ...
  const fetchCrewMembers = async () => {
    ...
    // Try to save sample data to Netlify Blobs for future use
    try {
      const store = getCrewStore();
      if (store) {
        await store.setJSON("crew-members", sampleData);
        console.log("✅ Sample data saved to Netlify Blobs");
      }
    } catch (error) {
      console.log("❌ Could not save sample data to Netlify Blobs");
    }
